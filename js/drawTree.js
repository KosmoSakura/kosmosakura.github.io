...
let drawInit = ($ele, options = {}) => {
    // let html = '<div id="drawArea" style="position: absolute; width: 100%;height: 100%;"></div>';
    // $ele.append(html);
    let {width, height} = options;

    let stretch = 600 / height,
        y_speed = 3 / stretch;
    let $statMsg = $("#statmsg");
    let $canvas = $('#canvas')[0];
    $canvas.width = width;
    ...
    let tree = new Tree();
    tree.init(ctx);
    extracted(width * 0.1, height, y_speed, stretch, tree);
    extracted(width * 0.2, height, y_speed, stretch, tree);
    extracted(width * 0.3, height, y_speed, stretch, tree);
    extracted(width * 0.5, height, y_speed, stretch, tree);
    extracted(width * 0.7, height, y_speed, stretch, tree);
    extracted(width * 0.8, height, y_speed, stretch, tree);
    extracted(width * 0.9, height, y_speed, stretch, tree);
...
function extracted(width, height, y_speed, stretch, tree) {
    new Branch({
        p: new Vector(width, height),
